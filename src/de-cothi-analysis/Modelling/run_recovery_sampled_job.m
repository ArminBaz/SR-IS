function run_recovery_sampled_job(input)
% RUN_RECOVERY_SAMPLED_JOB  Model recovery using population-sampled parameters.
%
%   input = [model_idx, species_idx, sim_idx]
%   model_idx  : 1=MF  2=MB  3=SR  4=Hybrid  5=SR-IS  (generating model)
%   species_idx: 1=human  2=rat  (which species' param distribution to sample from)
%   sim_idx    : 1:25  (which sample — unique RNG seed per job)
%
%   Checkpoints saved to checkpoints_sampled/{model}_{species}_sim{k}.mat
...
if ~exist('checkpoints_sampled', 'dir'), mkdir('checkpoints_sampled'); end

ckpt_file = sprintf('checkpoints_sampled/%s_%s_sim%d.mat', model_name, species, sim_idx);
if exist(ckpt_file, 'file')
    fprintf('Already done: %s %s sim%d\n', model_name, species, sim_idx);
    return;
end

fprintf('Starting: %s %s sim%d\n', model_name, species, sim_idx);
...
% -------------------------------------------------------------------------
%  Sample parameters for this job
%  RNG seed = model_idx*1000 + species_idx*100 + sim_idx (unique per job)
% -------------------------------------------------------------------------
rng_seed = model_idx * 1000 + species_idx * 100 + sim_idx;
[params_all, param_median, param_sem] = sample_model_params(model_idx, species_idx, 1, rng_seed);
params = params_all(1,:);

fprintf('  Sampled params: ');
fprintf('%.4f  ', params);
fprintf('\n  (median: ');
fprintf('%.4f  ', param_median);
fprintf('| sem: ');
fprintf('%.4f  ', param_sem);
fprintf(')\n');
...
% Use participant 1 start states from whichever real dataset
if species_idx == 1
    load('humans.mat'); dat_ref = humans;
else
    tmp = load('rat.mat'); dat_ref = tmp.rat;
end
...
[~, winner] = min(BIC_r);
fprintf('  %s %s sim%d --> Winner: %s\n', model_name, species, sim_idx, model_names{winner});

save(ckpt_file, 'sim_ll_r', 'params');
fprintf('Done: %s %s sim%d\n', model_name, species, sim_idx);
end
